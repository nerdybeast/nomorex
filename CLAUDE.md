...
- **1RM is derived, not stored — and may be estimated.** `oneRepMaxProvider` maps `exerciseId -> OneRepMax` (`lib/shared/models/one_rep_max.dart`), preferring the heaviest `personal_bests` row with `reps = 1` and falling back to the best estimate from multi-rep PRs when the user has never tested a single. `OneRepMax.isEstimated` records which, and **every surface that renders a weight resolved from a 1RM must carry that flag through** — `formatResolvedWeight()` (`core/utils/weight_converter.dart`) appends the ` (est.)` marker, so use it rather than `formatWeightForPreference` for a resolved percentage weight. Estimation uses the user's `profiles.one_rep_max_formula` (Brzycki/Epley/Lander, default Brzycki) via `estimateOneRepMaxKg()` in `core/utils/one_rep_max_formula.dart`, and is withheld entirely outside 2..`kMaxEstimableReps` (10) reps — a lift with only high-rep PRs is simply absent from the map, the same "no 1RM" state as having none. Both 1RM providers `ref.watch(oneRepMaxFormulaProvider)`, so changing the setting recomputes them; don't add a manual invalidation. Personal bests remain the source of truth for programming percentages.
- **An `exercises.id` only means something inside one user's catalog — across users, match on name.** Two users who both have a custom "dude pulls" have two unrelated rows with different ids (there's no unique constraint on `name`, so duplicates by name are expected, not a data error). This matters everywhere a viewer looks at someone else's data: a public workout's `workout_sets.basis_exercise_id` points at the *owner's* exercise, while the viewer's PR for the same lift hangs off *their* id, so an id-keyed lookup silently misses forever. Resolve a viewer-facing 1RM through `lookupOneRepMax()` (`features/workouts/utils/set_resolver.dart`), which tries `oneRepMaxProvider` by id and then falls back to `oneRepMaxByNameProvider` by lowercased name; when a viewer needs their *own* row for a foreign exercise, go through `ExercisesNotifier.ensureExerciseByName()`, which find-or-creates by name. The two halves must agree on name matching or a user can record a PR that never resolves anything.
