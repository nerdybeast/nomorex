...
- **Set-builder shorthand.** `parseSetBuilder()` accepts `<sets> x <reps> @ <pct>[-<pct>...]` (e.g. `3x5@80`, `4x2@65-68-71-68`) and throws `FormatException` with user-facing messages. A single percentage applies to all sets; a hyphenated list must have exactly one value per set.
